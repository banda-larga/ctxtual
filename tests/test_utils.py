...
@pytest.fixture
def papers_forge(sample_papers: list[dict]) -> tuple[Ctx, str]:
    """Ctx with a pre-populated papers workspace."""
    ...
    ctx.store.set_items(ws_id, sample_papers)
    return ctx, ws_id
...
class TestPaginator:
    def test_paginate_first_page(self, papers_forge) -> None:
        ctx, ws_id = papers_forge
        ts = paginator(ctx, "papers")
...
class TestTextSearch:
    def test_search_by_title(self, papers_forge) -> None:
        ctx, ws_id = papers_forge
        ts = text_search(ctx, "papers", fields=["title"])
    ...
    def test_search_max_results(self, papers_forge) -> None:
        ctx, ws_id = papers_forge
        ts = text_search(ctx, "papers", fields=["abstract"])
...
class TestKVReader:
    ...
    def test_get_value(self) -> None:
        ...
        meta = WorkspaceMeta(workspace_id="ws_cfg", workspace_type="config")
        ctx.store.init_workspace(meta)
        ctx.store.set_items("ws_cfg", {"name": "app", "version": "1.0"})
    ...
    def test_get_value_missing_key(self) -> None:
        ...
        meta = WorkspaceMeta(workspace_id="ws_cfg", workspace_type="config")
        ctx.store.init_workspace(meta)
        ctx.store.set_items("ws_cfg", {"name": "app"})
