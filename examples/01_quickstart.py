...
@ctx.producer(workspace_type="items", toolsets=[paginator(ctx), text_search(ctx)])
...
def agent_loop(user_message: str):
    ...
    The structure stays the same:
      response = llm.chat(messages, tools=ctx.get_tools())
      for tool_call in response.tool_calls:
          result = ctx.dispatch_tool_call(tool_call.name, tool_call.args)
    """
